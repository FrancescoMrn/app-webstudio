...
| Version | Changes |
|---------|---------|
| 1.1.0 | First-class website projects, self-contained in the app. Where 1.0.0 delegated the editor, files, preview, build, and publish to a platform Projects subsystem that isn't available on every shell, 1.1.0 restores and modularizes the full in-app IDE on top of stable Möbius primitives (storage, embedded chat, build runner, `/sites/` publish). The project *list* now uses the shared `window.mobius.projects` registry when the shell provides it, with a `projects.json` fallback for older shells. Adds a preview element-comment tool (click an element, queue notes, and send them to the agent as one message) and auto-refresh of the preview after each agent turn. |
