...
export default function App({ appId, token }) {
  ...
  // Preview annotation tool (pencil): click an element in the preview to attach
  // a comment, then send all pending comments to the embedded agent chat.
  const [annotating, setAnnotating] = useState(false)
  const [annotations, setAnnotations] = useState([]) // {id, selector, tag, text, comment}
  const [chatId, setChatId] = useState(null)
  const [sendingComments, setSendingComments] = useState(false)
  const [sentNotice, setSentNotice] = useState(0) // count just delivered (0 = none)
  const handleAnnotate = useCallback((a) => {
    setAnnotations((prev) => [
      ...prev,
      { id: `${Date.now()}-${prev.length}`, selector: a.selector, tag: a.tag, text: a.text, comment: '' },
    ])
  }, [])
  const setAnnotationComment = useCallback((id, comment) => {
    setAnnotations((prev) => prev.map((x) => (x.id === id ? { ...x, comment } : x)))
  }, [])
  ...
  // The project *list* is a platform primitive now: window.mobius.projects owns
  // it (a shared "named workspaces" registry), so several apps can offer
  // projects without hand-rolling one. This app still owns each project's FILES
  // in its own storage under projectPrefix(id). Older shells without the
  // primitive fall back to the previous app-storage projects.json registry.
  const projectsApi = useMemo(() => {
    const mp = (typeof window !== 'undefined' && window.mobius && window.mobius.projects) || null
    const toRow = (p) => ({
      id: p.id,
      name: p.name,
      createdAt: Number.isFinite(Date.parse(p.created_at)) ? Date.parse(p.created_at) : Date.now(),
    })
    if (mp) {
      return {
        list: async () => normalizeProjects((await mp.list()).map(toRow)) || [],
        create: async (name, id) => toRow(await mp.create(name, id)),
        rename: (id, name) => mp.rename(id, name),
        remove: (id) => mp.remove(id),
      }
    }
    // Fallback: the legacy in-app registry (older shells, no primitive).
    const readList = async () => normalizeProjects(await rootStorage.getFresh('projects.json')) || []
    const write = async (mutate) => {
      const { value } = await rootStorage.updateJSON('projects.json', (current) => {
        const norm = normalizeProjects(current)
        const base = norm && norm.length
          ? norm
    ...
    return {
      ...
      create: async (name, id) => {
        const pid = id || projectSlug(name, new Set((await readList()).map((p) => p.id)))
        const existing = (await readList()).find((p) => p.id === pid)
        if (existing) return existing
        const row = { id: pid, name, createdAt: Date.now() }
        await write((base) => [...base, row])
        return row
      },
      rename: async (id, name) => { await write((base) => base.map((p) => (p.id === id ? { ...p, name } : p))) },
      remove: async (id) => { await write((base) => base.filter((p) => p.id !== id)) },
    }
  }, [rootStorage])
  ...
  const refreshProjects = useCallback(async () => {
    ...
    const next = (list && list.length) ? list : [{ id: DEFAULT_PROJECT.id, name: DEFAULT_PROJECT.name, createdAt: Date.now() }]
    setProjects(next)
    return next
  }, [projectsApi])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        // Ensure a durable "default" project exists (idempotent), then load the
        // list from the platform registry.
        await projectsApi.create(DEFAULT_PROJECT.name, DEFAULT_PROJECT.id).catch(() => {})
  ...
  }, [appId, activeProjectId, projectsApi, refreshProjects])
  ...
  // Rebuild the preview after each agent turn (set once handleBuild exists).
  const autoBuildRef = useRef(null)
  const onFilesMaybeChanged = useCallback(async () => {
    // Never let a sync failure skip the rebuild below — the whole point is to
    // refresh the preview after the agent's turn, so it must always run.
    try { await syncProjectFromStorage() } catch { /* rebuild anyway */ }
    ...
    // The agent likely edited files this turn; rebuild so the preview shows the
    // latest result without the user tapping Build. A short delay lets any
    // trailing storage writes from the turn settle before build.sh reads them.
    // handleBuild no-ops when there's no main page or a build is already running.
    setTimeout(() => { autoBuildRef.current?.() }, 400)
  ...
  const commitRenameProject = useCallback(async (targetId, rawName) => {
    ...
    try {
      if (!clean) return
      await projectsApi.rename(targetId, clean)
  ...
  }, [modal, projectsApi, refreshProjects])
  ...
  const createAndRenameProject = useCallback(async () => {
    ...
    try {
      const current = await projectsApi.list()
      const name = `Project ${current.length + 1}`
  ...
  }, [modal, projectsLoaded, switchProject, projectsApi, refreshProjects])
  ...
  const handleDeleteProject = useCallback(async (targetId) => {
    ...
    try {
      ...
      // Remove the registry entry (platform primitive), then this app's files.
      await projectsApi.remove(targetId)
  ...
  // Expose the latest handleBuild to onFilesMaybeChanged (defined earlier) so a
  // finished agent turn can auto-rebuild the preview.
  useEffect(() => { autoBuildRef.current = handleBuild }, [handleBuild])

  // Send all pending annotation comments to the embedded agent chat as one
  // message (manual — only on the user's tap). The app owns this chat, so its
  // app token may post to it; the embedded ChatView shows the turn.
  const sendComments = useCallback(async () => {
    if (sendingComments || annotations.length === 0) return
    setSendingComments(true)
    try {
      // Resolve the chat id: the live handle if the chat is mounted, else the
      // persisted id. If neither exists the chat was never opened this project,
      // so open it (which creates it) and ask the user to send again.
      let cid = chatId
      if (!cid) {
        try {
          const p = await storage.get('chat_id.json')
          cid = (p && (p.id || p.chatId)) || (typeof p === 'string' ? p : null)
        } catch { /* none persisted */ }
      }
      if (!cid) {
        if (!chatOpen) toggleChat()
        await modal.alert('Opening the chat — tap “Send to agent” again once it loads.', { title: 'Almost there' })
        return
      }
      const lines = annotations.map((a, i) => {
        const loc = a.text ? `“${a.text}” (${a.tag || 'element'})` : (a.selector || a.tag || 'element')
        return `${i + 1}. ${loc}${a.selector ? ` [${a.selector}]` : ''}\n   → ${(a.comment || '').trim() || '(no note)'}`
      })
      const content = `I left ${annotations.length} comment(s) on the page — please apply them:\n\n${lines.join('\n\n')}`
      const res = await fetch(`/api/chats/${encodeURIComponent(cid)}/messages`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ content, cid: `ws-annot-${Date.now()}` }),
      })
      ...
      setAnnotating(false)
      setSentNotice(count)
      signal('annotations_sent', { count })
      if (!chatOpen) toggleChat()
    } catch (e) {
      await modal.alert(e.message || 'Could not send comments.', { title: 'Send failed' })
    } finally {
      setSendingComments(false)
    }
  }, [chatId, sendingComments, annotations, token, chatOpen, toggleChat, modal, storage])

  // The "delivered" confirmation clears itself after a few seconds.
  useEffect(() => {
    if (!sentNotice) return undefined
    const t = setTimeout(() => setSentNotice(0), 4500)
    return () => clearTimeout(t)
  }, [sentNotice])

  // The pencil tool's comment tray, overlaid on the bottom of the preview.
  function renderAnnotationPanel() {
    const btn = (primary) => ({
      font: '12px system-ui, sans-serif',
      color: '#fff',
      background: primary ? 'var(--accent, #7c6cff)' : 'transparent',
      border: `1px solid ${primary ? 'var(--accent, #7c6cff)' : 'rgba(255,255,255,0.18)'}`,
      borderRadius: 6, padding: '4px 10px', cursor: 'pointer',
    })
    return (
      <div style={{
        position: 'absolute', left: 0, right: 0, bottom: 0, maxHeight: '55%', overflowY: 'auto',
        background: 'var(--surface, rgba(18,18,24,0.97))', borderTop: '1px solid rgba(255,255,255,0.12)',
        padding: '10px 12px', font: '13px system-ui, sans-serif', color: 'var(--text, #eee)', zIndex: 5,
      }}>
        {sentNotice > 0 && (
          <div role="status" style={{
            display: 'flex', alignItems: 'center', gap: 8, marginBottom: annotations.length || annotating ? 8 : 0,
            padding: '8px 10px', borderRadius: 8,
            background: 'rgba(46, 160, 87, 0.18)', border: '1px solid rgba(46,160,87,0.5)',
            color: '#7ee2a0', fontSize: 13,
          }}>
            <span aria-hidden="true" style={{ fontWeight: 700 }}>✓</span>
            <span>
              Delivered {sentNotice} comment{sentNotice === 1 ? '' : 's'} to the agent — watch the chat for the update.
            </span>
          </div>
        )}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, gap: 8 }}>
          <strong style={{ fontSize: 12 }}>
            {annotating ? 'Click an element in the preview to comment on it' : 'Comments'}
          </strong>
          <div style={{ display: 'flex', gap: 8, flex: '0 0 auto' }}>
            {annotations.length > 0 && (
              <button type="button" onClick={sendComments} disabled={sendingComments}
                style={btn(true)} title="Send comments to the agent">
                {sendingComments ? 'Sending…' : `Send ${annotations.length} to agent`}
              </button>
            )}
            <button type="button" onClick={() => setAnnotating(false)} style={btn(false)}>Done</button>
          </div>
        </div>
        {annotations.length === 0 ? (
          <div style={{ opacity: 0.6 }}>No comments yet — click something in the preview.</div>
        ) : (
          <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {annotations.map((a, i) => (
              <li key={a.id} style={{ border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, padding: 8 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 6 }}>
                  <span style={{ fontSize: 12, opacity: 0.85, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {i + 1}. {a.text ? `“${a.text}”` : (a.selector || a.tag)} <em style={{ opacity: 0.6 }}>({a.tag || 'element'})</em>
                  </span>
                  <button type="button" onClick={() => removeAnnotation(a.id)} style={{ ...btn(false), padding: '2px 7px' }} aria-label="Remove comment">✕</button>
                </div>
                <input
                  type="text"
                  value={a.comment}
                  placeholder="What should change here?"
                  onChange={(e) => setAnnotationComment(a.id, e.target.value)}
                  style={{
                    ...
                    color: 'inherit', font: '13px system-ui, sans-serif',
                  }}
                />
              </li>
            ))}
  ...
  function renderPreviewView() {
    ...
    if (entryForMain) {
      return (
        <div className="ws-preview-wrap" style={{ position: 'relative', width: '100%', height: '100%' }}>
          <HtmlPreview
            storage={storage}
          ...
          />
          {(annotating || annotations.length > 0 || sentNotice > 0) && renderAnnotationPanel()}
        </div>
      )
    }
    return (
    ...
    <div className="ws-root">
      ...
      <header className="ws-top-bar">
        ...
        <div className="ws-top-zone ws-top-zone--right">
          ...
          {mainPath && entryForMain && (isWide || viewMode === 'preview') && (
            <button
              type="button"
              className={`ws-toolbar-btn ${annotating ? 'ws-toolbar-btn--primary' : ''}`}
              onClick={() => setAnnotating((v) => !v)}
              aria-pressed={annotating}
              aria-label={annotating ? 'Stop commenting' : 'Comment on the page'}
              title={annotating ? 'Stop commenting' : 'Comment on the page (click an element)'}
            >
              <PencilIcon size={20} />
            </button>
          )}
