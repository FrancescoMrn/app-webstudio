...
// Annotation mode: the pencil tool. The parent postMessages {type:ws-annotate-mode,
// on} to toggle it (the sandboxed preview has no allow-same-origin, so the parent
// can only reach it by postMessage). While ON, hovering outlines the element under
// the cursor and a click captures a stable locator + its text and posts it back so
// the app can attach a comment. All native behavior is suppressed while annotating.
export const WS_PREVIEW_ANNOTATE_TYPE = 'ws-preview-annotate'
export const WS_PREVIEW_ANNOTATE_MODE_TYPE = 'ws-annotate-mode'
export const WS_PREVIEW_ANNOTATE_SCRIPT = `
(function () {
  var on = false;
  var last = null;
  function clearHl() {
    if (last && last.style) { last.style.outline = last.__wsOldOutline || ''; last.style.cursor = ''; }
    last = null;
  }
  function cssPath(el) {
    ...
    while (el && el.nodeType === 1 && el.tagName && el.tagName.toLowerCase() !== 'html') {
      ...
      var idx = 1, n = 0, p = el.parentNode;
      if (p && p.children) {
        for (var i = 0; i < p.children.length; i++) {
          if (p.children[i].tagName === el.tagName) { n++; if (p.children[i] === el) idx = n; }
        }
      }
  ...
  window.addEventListener('message', function (e) {
    if (!e.data || e.data.type !== '${WS_PREVIEW_ANNOTATE_MODE_TYPE}') return;
    on = !!e.data.on;
    if (!on) clearHl();
  ...
  document.addEventListener('mouseover', function (e) {
    ...
    clearHl();
    last = e.target;
    if (last && last.style) { last.__wsOldOutline = last.style.outline; last.style.outline = '2px solid #7c6cff'; }
  }, true);
  document.addEventListener('mouseout', function () { if (on) clearHl(); }, true);
