...
import {
  ...
  WS_PREVIEW_NAV_TYPE,
  WS_PREVIEW_ANNOTATE_SCRIPT,
  WS_PREVIEW_ANNOTATE_TYPE,
  WS_PREVIEW_ANNOTATE_MODE_TYPE,
} from './previewDomain.js'

export function HtmlPreview({ storage, entryPath, version, annotating = false, onAnnotate }) {
  ...
  useEffect(() => {
    const onMessage = (event) => {
      ...
      if (!data) return
      if (data.type === WS_PREVIEW_ANNOTATE_TYPE) {
        // The pencil tool captured an element click inside the preview.
        if (typeof onAnnotateRef.current === 'function') {
      ...
      if (data.type !== WS_PREVIEW_NAV_TYPE) return
  ...
  // Keep the latest onAnnotate without re-installing the message listener.
  const onAnnotateRef = useRef(onAnnotate)
  useEffect(() => { onAnnotateRef.current = onAnnotate }, [onAnnotate])

  // Toggle annotate mode inside the sandboxed frame (postMessage is the only
  // channel — the frame has no allow-same-origin). Re-post on load + toggle.
  useEffect(() => {
    const frame = frameRef.current
    if (!frame || !frame.contentWindow) return
    try {
      frame.contentWindow.postMessage(
        { type: WS_PREVIEW_ANNOTATE_MODE_TYPE, on: !!annotating }, '*',
      )
    } catch { /* frame not ready; onLoad re-posts below */ }
  }, [annotating, srcDoc])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // The pencil annotation tool (dormant until the parent toggles it on).
        const annotateScript = doc.createElement('script')
        annotateScript.textContent = WS_PREVIEW_ANNOTATE_SCRIPT
        ;(doc.body || doc.documentElement).appendChild(annotateScript)
