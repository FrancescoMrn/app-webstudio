        ...
        <>
          ...
          <div className="ws-drawer-publish-actions">
            <button
              ...
              onClick={() => {
                // The app runs in an opaque-origin frame where navigator.clipboard
                // is blocked. window.mobius.clipboard delegates to the host (with
                // an execCommand fallback), which actually copies. Fall back to
                // navigator.clipboard only if the runtime bridge is absent.
                const value = publishedUrl
                const host = typeof window !== 'undefined' && window.mobius?.clipboard
                if (host?.writeText) host.writeText(value).catch(() => {})
